...
function App() {
  ...
  const fraud = async target => {
    try {
      ...
      setData(prev => ({ ...prev, users: prev.users.map(u => docId(u) === docId(target) ? result.user : u) }));
    } catch { setData(prev => ({ ...prev, users: prev.users.map(u => docId(u) === docId(target) ? { ...u, role: 'user', fraud: true } : u) })); }
  };
  ...
  return <div className={dark ? 'app dark' : 'app'}>
    ...
    <main>
      ...
      {route.startsWith('/tickets/') && <Guard user={user} go={go}><Details ticket={data.tickets.find(t => docId(t) === route.split('/').pop())} open={setModal} /></Guard>}
...
function Navbar({ user, go, logout, dark, setDark, menu, setMenu, apiOnline }) {
  return <header className="topbar"><button className="icon mobile" onClick={() => setMenu(true)}><Menu size={20} /></button><button className="brand" onClick={() => go('/')}><span><Bus size={20} /></span>TicketBari</button><nav className={menu ? 'nav open' : 'nav'}><button className="icon close" onClick={() => setMenu(false)}><X size={18} /></button><button onClick={() => go('/')}>Home</button><button onClick={() => go('/tickets')}>All Tickets</button>{user && <button onClick={() => go(`/dashboard/${user.role}`)}>Dashboard</button>}</nav><div className="actions"><small title={apiOnline ? 'Connected to backend' : 'Local fallback'}>{apiOnline ? 'API' : 'LOCAL'}</small><button className="icon" onClick={() => setDark(!dark)}>{dark ? <Sun size={18} /> : <Moon size={18} />}</button>{!user ? <><button className="ghost" onClick={() => go('/login')}>Login</button><button className="primary" onClick={() => go('/register')}>Register</button></> : <div className="account"><span className="avatar">{user.avatar || initials(user.name)}</span><b>{user.name}</b><ChevronDown size={14} /><div><button onClick={() => go(`/dashboard/${user.role}`)}><UserRound size={15} /> My Profile</button><button onClick={logout}><LogOut size={15} /> Logout</button></div></div>}</div></header>;
}
function Home({ tickets, go }) { return <><section className="hero"><div><p className="eyebrow">Travel without queue stress</p><h1>Book verified bus, train, launch and flight tickets in one calm place.</h1><p>TicketBari brings approved vendors, clear pricing, booking requests, payments and role dashboards into one responsive MERN platform.</p><div><button className="primary" onClick={() => go('/tickets')}>Find tickets</button><button className="ghost light" onClick={() => go('/login')}>Try demo roles</button></div></div></section><Section title="Advertised tickets" subtitle="Exactly six admin-picked offers appear here."><TicketGrid tickets={tickets.filter(t => t.advertised).slice(0, 6)} go={go} /></Section><Section title="Latest tickets" subtitle="Recently added and admin-approved journeys."><TicketGrid tickets={[...tickets].sort((a, b) => new Date(b.createdAt) - new Date(a.createdAt)).slice(0, 8)} go={go} /></Section><section className="split"><div><p className="eyebrow">Popular routes</p><h2>Dhaka to Coxs Bazar, Dhaka to Chattogram, Sylhet to Dhaka.</h2><p>Search by origin and destination, filter by transport type, then compare prices without losing the rhythm of the page.</p></div><div className="why">{['Verified vendors', 'Payment-ready booking', 'Countdown reminders', 'Responsive dashboard'].map(x => <span key={x}><BadgeCheck size={17} />{x}</span>)}</div></section></>; }
...
function TicketCard({ ticket, go }) { return <article className="ticket-card"><img src={ticket.image} alt={ticket.title} /><div><div className="title-row"><h3>{ticket.title}</h3><span><Transport type={ticket.type} />{ticket.type}</span></div><p>{ticket.from} to {ticket.to}</p><div className="meta"><b>{money(ticket.price)}</b><span>{ticket.quantity} seats</span></div><div className="perks">{ticket.perks?.map(p => <small key={p}>{p}</small>)}</div><button className="primary wide" onClick={() => go(`/tickets/${docId(ticket)}`)}>See details</button></div></article>; }
function AllTickets({ tickets, go }) {
  const [q, setQ] = useState(''), [type, setType] = useState('All'), [sort, setSort] = useState('recent'), [page, setPage] = useState(1);
  const filtered = useMemo(() => { const list = tickets.filter(t => `${t.from} ${t.to}`.toLowerCase().includes(q.toLowerCase()) && (type === 'All' || t.type === type)); if (sort === 'low') list.sort((a, b) => a.price - b.price); if (sort === 'high') list.sort((a, b) => b.price - a.price); if (sort === 'recent') list.sort((a, b) => new Date(b.createdAt) - new Date(a.createdAt)); return list; }, [tickets, q, type, sort]);
  useEffect(() => setPage(1), [q, type, sort]); const pages = Math.max(1, Math.ceil(filtered.length / 6)); const visible = filtered.slice((page - 1) * 6, page * 6);
  return <section className="section page"><div className="page-title"><h1>All Tickets</h1><p>Search by route, filter transport, sort by price and browse with pagination.</p></div><div className="filters"><label><Search size={18} /><input value={q} onChange={e => setQ(e.target.value)} placeholder="Search From - To" /></label><select value={type} onChange={e => setType(e.target.value)}><option>All</option><option>Bus</option><option>Train</option><option>Launch</option><option>Plane</option></select><select value={sort} onChange={e => setSort(e.target.value)}><option value="recent">Newest first</option><option value="low">Price low to high</option><option value="high">Price high to low</option></select></div><TicketGrid tickets={visible} go={go} /><div className="pagination">{Array.from({ length: pages }, (_, i) => <button key={i} className={page === i + 1 ? 'active' : ''} onClick={() => setPage(i + 1)}>{i + 1}</button>)}</div></section>;
}
...
function Dashboard({ role, user, data, updateTicket, updateBooking, addTicket, pay, setRole, fraud, api }) {
  if (role !== user.role && user.role !== 'admin') return <ErrorPage />; const ticketsById = Object.fromEntries(data.tickets.map(t => [docId(t), t])); const vendorTickets = data.tickets.filter(t => String(docId(t.vendor) || t.vendor) === String(docId(user)) && t.status !== 'deleted'); const vendorBookings = data.bookings.filter(b => vendorTickets.some(t => docId(t) === (docId(b.ticket) || b.ticket))); const sold = vendorBookings.filter(b => b.status === 'paid').reduce((s, b) => s + b.quantity, 0); const revenue = vendorBookings.filter(b => b.status === 'paid').reduce((s, b) => s + (ticketOf(b, data.tickets)?.price || 0) * b.quantity, 0);
  return <section className="dashboard"><aside><h2><LayoutDashboard size={18} />{role} dashboard</h2>{(role === 'user' ? ['User Profile', 'My Booked Tickets', 'Transaction History'] : role === 'vendor' ? ['Vendor Profile', 'Add Ticket', 'My Added Tickets', 'Requested Bookings', 'Revenue Overview'] : ['Admin Profile', 'Manage Tickets', 'Manage Users', 'Advertise Tickets']).map(x => <a key={x} href={`#${x.replaceAll(' ', '-').toLowerCase()}`}>{x}</a>)}</aside><div className="dash-main"><Panel title={`${role[0].toUpperCase()}${role.slice(1)} Profile`}><Profile user={user} /></Panel>{role === 'user' && <><Panel id="my-booked-tickets" title="My Booked Tickets"><BookingCards bookings={data.bookings.filter(b => String(docId(b.user) || b.user) === String(docId(user)))} tickets={ticketsById} pay={pay} /></Panel><Panel id="transaction-history" title="Transaction History"><Transactions items={data.transactions.filter(t => String(docId(t.user) || t.user) === String(docId(user)))} /></Panel></>}{role === 'vendor' && <><Panel id="add-ticket" title="Add Ticket"><TicketForm user={user} add={addTicket} api={api} /></Panel><Panel id="my-added-tickets" title="My Added Tickets"><VendorTickets tickets={vendorTickets} updateTicket={updateTicket} /></Panel><Panel id="requested-bookings" title="Requested Bookings"><BookingTable bookings={vendorBookings} tickets={ticketsById} updateBooking={updateBooking} /></Panel><Panel id="revenue-overview" title="Revenue Overview"><Revenue tickets={vendorTickets.length} sold={sold} revenue={revenue} /></Panel></>}{role === 'admin' && <><Panel id="manage-tickets" title="Manage Tickets"><ManageTickets tickets={data.tickets} updateTicket={updateTicket} /></Panel><Panel id="manage-users" title="Manage Users"><ManageUsers users={data.users} setRole={setRole} fraud={fraud} current={docId(user)} /></Panel><Panel id="advertise-tickets" title="Advertise Tickets"><Advertise tickets={data.tickets.filter(t => t.status === 'approved')} updateTicket={updateTicket} /></Panel></>}</div></section>;
}
...
function Revenue({ tickets, sold, revenue }) { return <div className="revenue"><div><span>Total tickets added</span><b>{tickets}</b></div><div><span>Total tickets sold</span><b>{sold}</b></div><div><span>Total revenue</span><b>{money(revenue)}</b></div><div className="chart">{[tickets * 12, sold * 18, Math.min(revenue / 150, 100)].map((h, i) => <i key={i} style={{ height: `${Math.max(h, 12)}%` }} />)}</div></div>; }
